...
public class TodaysEventController extends AnchorPane {
    ...
    private void setNeedArticles(Event event) {
        if (event != null) {
            Collection<EventArticle> alreadyBlockedArticlesForEvents = DatabaseController.loadBy(Arrays.asList(event.getId()));
            Collection<Article> articles = DatabaseController.findBy(alreadyBlockedArticlesForEvents.stream().map(EventArticle::getArticleId).collect(Collectors.toSet()));
